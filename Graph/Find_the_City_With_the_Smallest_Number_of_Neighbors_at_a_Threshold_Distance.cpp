...
int findTheCity(int n, vector<vector<int>>& edges, int distanceThreshold) 
{
    ...
    for(int i = 0 ; i < n ; i++)
    {
        for(int j = 0 ; j< n; j++)
        {
            for(int k = 0 ; k<n;k++)
            {
                ...
                if(mat[j][i]==INT_MAX || mat[i][k]==INT_MAX) continue;

                if(mat[j][i]+mat[i][k]<=distanceThreshold) mat[j][k]=min(mat[j][k],mat[j][i]+mat[i][k]);
            }
        }
    }
    ...
    for(int i = 0 ; i< n ; i++)
    {
        ...
        for(int j = 0 ; j< n ;j++)
        {
            if(mat[i][j]!=INT_MAX and mat[i][j]<=distanceThreshold and mat[i][j]) count++;
        }
